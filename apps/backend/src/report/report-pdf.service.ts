...
export class ReportPdfService {
  ...
  async generateReportPdf(input: GeneratePdfInput): Promise<{ buffer: Buffer; pageCount: number }> {
    ...
    const revenueSeries = this.extractRevenueSeries(input.financialRows, input.annualRevenueSeries);
    ...
    // ═══════════════════════════════════════════════════════════════
    addPage();
    this.drawSectionPage(doc, localized.summaryTitle, localized.summarySubtitle, contentW);
    const execSummary = input.aiContent['EXECUTIVE_SUMMARY'] || input.aiContent['SECTION_EXECUTIVE_SUMMARY'] || 'AI-generated summary is currently unavailable.';
    ...
    for (const type of input.reportTypes) {
      ...
      const sectionText = input.aiContent[type] || `AI analysis for ${prettyName} is being processed.`;
      this.renderMarkdownContent(doc, sectionText, contentW);

      if (type === 'FINANCIAL' && input.includeCharts) {
        this.ensureSpace(doc, 220, () => { addPage(); });
        doc.moveDown(1.5);
    ...
    // ═══════════════════════════════════════════════════════════════
    const sectionKeys = Object.keys(input.aiContent).filter(k =>
      k.startsWith('SECTION_') && k !== 'SECTION_EXECUTIVE_SUMMARY' && input.aiContent[k],
    );
    ...
    if (input.problemStatement) {
      ...
      // Draw focus box
      const focusY = doc.y;
      doc.roundedRect(56, focusY, contentW, 3, 1).fill(THEME.accent);
      doc.moveDown(0.5);

      const label = input.language === 'FR' ? 'Contexte soumis par l\'utilisateur :' :
        input.language === 'AR' ? 'السياق المقدم من قبل المستخدم :' :
          'User-provided context:';

      doc.font(FONTS.heading).fontSize(10).fillColor(THEME.textSecondary).text(label, 56, doc.y);
      doc.moveDown(0.5);
    ...
    // ═══════════════════════════════════════════════════════════════
    addPage();
    this.drawSectionPage(doc, localized.appendixTitle, localized.appendixSubtitle, contentW);
    ...
    const unusedKeys = Object.keys(input.aiContent).filter(k => {
      return !input.reportTypes.includes(k) &&
        k !== 'EXECUTIVE_SUMMARY' &&
    ...
    for (const key of unusedKeys) {
      addPage();
      this.drawSectionPage(doc, this.prettySectionKey(key, input.language), '', contentW);
      this.renderMarkdownContent(doc, input.aiContent[key], contentW);
    }
  ...
  private drawCoverPage(
    ...
    doc.rect(0, 0, pageW, pageH).fill(THEME.coverBg);

    // Decorative accent stripe at top
    doc.rect(0, 0, pageW, 6).fill(THEME.accent);

    // Decorative geometric elements
    doc.save();
    doc.opacity(0.05);
    doc.circle(pageW - 80, 120, 200).fill(THEME.accentLight);
    doc.circle(60, pageH - 100, 150).fill(THEME.accentLight);
    doc.restore();

    // Left accent bar
    doc.rect(56, 160, 4, 80).fill(THEME.accent);

    // SmartBiz AI logo text
    doc.font(FONTS.heading).fontSize(14).fillColor(THEME.accentLight)
      .text('SMARTBIZ AI', 56, 120, { characterSpacing: 4 });

    // Main title
    doc.font(FONTS.heading).fontSize(32).fillColor(THEME.textWhite)
      .text(localized.reportTitle, 72, 175, { width: pageW - 150 });

    // Subtitle
    doc.moveDown(0.5);
    doc.font(FONTS.body).fontSize(14).fillColor(THEME.textOnDark)
      .text(localized.reportSubtitle, 72, doc.y, { width: pageW - 150 });

    // Divider line
    doc.moveTo(56, 310).lineTo(pageW - 56, 310).strokeColor(THEME.primaryLight).lineWidth(0.5).stroke();

    // Metadata section with cards
    const metaY = 340;
    doc.font(FONTS.heading).fontSize(11).fillColor(THEME.accentLight)
      .text(localized.metadata.toUpperCase(), 56, metaY, { characterSpacing: 2 });
    ...
    for (const [label, value] of metaItems) {
      doc.fillColor(THEME.textLight).text(`${label}:`, 72, metaRowY, { continued: true });
      doc.fillColor(THEME.textOnDark).text(`  ${value}`, { continued: false });
      metaRowY += 18;
    }

    // Bottom decorative bar
    doc.rect(0, pageH - 40, pageW, 40).fill(THEME.primaryDark);
    doc.rect(0, pageH - 40, pageW, 2).fill(THEME.accent);

    // Bottom text
    const poweredByLabel = this.getPoweredByLabel(input.aiSource);
    doc.font(FONTS.body).fontSize(8).fillColor(THEME.textLight)
  ...
  private drawTableOfContents(
    ...
    // Section title
    this.drawSectionPage(doc, localized.contentsTitle, localized.contentsSubtitle, contentW);
    ...
    input.reportTypes.forEach((type, index) => {
      tocItems.push({ label: `${index + 2}. ${this.prettyType(type, input.language)}`, page: tocPage });
      tocPage += 1;
    });

    // Additional sections
    const sectionKeys = Object.keys(input.aiContent).filter(k => k.startsWith('SECTION_') && k !== 'SECTION_EXECUTIVE_SUMMARY');
    ...
    for (const item of tocItems) {
      ...
      if (tocItems.indexOf(item) % 2 === 0) {
        doc.rect(56, tocY - 4, contentW, 28).fill(THEME.sectionBg);
      }

      doc.font(FONTS.body).fontSize(11).fillColor(THEME.textPrimary)
        .text(item.label, 66, tocY, { width: contentW - 60 });

      // Page number on right
      doc.font(FONTS.heading).fontSize(11).fillColor(THEME.accent)
      ...
      if (dotStart < pageNumStart) {
        doc.font(FONTS.body).fontSize(9).fillColor(THEME.textLight);
        let dotX = dotStart;
  ...
  private drawSectionPage(
    ...
    doc.rect(56, startY, 4, 28).fill(THEME.accent);

    // Title
    doc.font(FONTS.heading).fontSize(20).fillColor(THEME.primary)
    ...
    doc.moveDown(0.6);
    const lineY = doc.y;
    doc.moveTo(56, lineY).lineTo(56 + contentW, lineY)
      .strokeColor(THEME.borderLight).lineWidth(1).stroke();
    // Accent portion of line
    doc.moveTo(56, lineY).lineTo(56 + 80, lineY)
  ...
  private renderMarkdownContent(doc: PDFKit.PDFDocument, text: string, contentW: number) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (trimmed.startsWith('## ')) {
        ...
        doc.rect(leftMargin, doc.y - 2, contentW, 26).fill(THEME.highlightBg);
        doc.rect(leftMargin, doc.y - 2, 3, 26).fill(THEME.accent);
        doc.font(FONTS.heading).fontSize(14).fillColor(THEME.primary)
          .text(headingText, leftMargin + 12, doc.y + 4, { width: contentW - 20 });
        doc.moveDown(0.8);
        continue;
      }
      ...
      if (trimmed.startsWith('### ')) {
        doc.moveDown(0.4);
        const headingText = trimmed.replace(/^###\s+/, '');
        doc.font(FONTS.heading).fontSize(12).fillColor(THEME.primaryLight)
      ...
      if (trimmed.startsWith('|') && trimmed.endsWith('|')) {
        ...
        this.renderTableRow(doc, trimmed, leftMargin, contentW, i === 0 || (i > 0 && lines[i - 1]?.trim().startsWith('|') === false));
        continue;
      }

      // Bullet point: - text or * text
      if (trimmed.match(/^[-*]\s/)) {
        const bulletText = this.stripMarkdownFormatting(trimmed.replace(/^[-*]\s+/, ''));
        const bulletY = doc.y;

        // Bullet dot
        doc.circle(leftMargin + 8, bulletY + 5, 2.5).fill(THEME.accent);

        // Check for bold prefix (like **Key**: value)
        this.renderFormattedText(doc, bulletText, leftMargin + 18, contentW - 24);
        doc.moveDown(0.15);
        continue;
      }
      ...
      if (trimmed.match(/^\s+[-*]\s/)) {
        const bulletText = this.stripMarkdownFormatting(trimmed.replace(/^\s+[-*]\s+/, ''));
        const bulletY = doc.y;

        doc.circle(leftMargin + 24, bulletY + 5, 2).fill(THEME.textMuted);
        this.renderFormattedText(doc, bulletText, leftMargin + 34, contentW - 40);
        doc.moveDown(0.15);
        continue;
      }
  ...
  private renderFormattedText(doc: PDFKit.PDFDocument, text: string, x: number, width: number) {
    ...
    for (const part of parts) {
      ...
      if (isFirst) {
        doc.font(isBold ? FONTS.heading : FONTS.body).fontSize(10)
          .fillColor(isBold ? THEME.primary : THEME.textPrimary)
          .text(cleanText, x, doc.y, { width, lineGap: 3, continued: parts.indexOf(part) < parts.length - 1 });
        isFirst = false;
      } else {
        doc.font(isBold ? FONTS.heading : FONTS.body).fontSize(10)
          .fillColor(isBold ? THEME.primary : THEME.textPrimary)
          .text(cleanText, { lineGap: 3, continued: parts.indexOf(part) < parts.length - 1 });
      }
    }
  }
  ...
  // TABLE RENDERER
  // ═══════════════════════════════════════════════════════════════
  private renderTableRow(doc: PDFKit.PDFDocument, row: string, x: number, contentW: number, isHeader: boolean) {
    ...
    if (isHeader) {
      ...
      doc.rect(x, y, contentW, rowH).fill(THEME.tableHeaderBg);
      doc.font(FONTS.heading).fontSize(8.5).fillColor(THEME.textWhite);
    } else {
      // Alternating body rows
      doc.rect(x, y, contentW, rowH).fill(THEME.sectionBg);
      doc.font(FONTS.body).fontSize(8.5).fillColor(THEME.textPrimary);
    }

    cells.forEach((cell, colIdx) => {
      const cleanCell = this.stripMarkdownFormatting(cell);
      doc.text(cleanCell, x + colIdx * colW + 6, y + 6, { width: colW - 12, lineBreak: false });
    });
  ...
  // ALERT BOX
  // ═══════════════════════════════════════════════════════════════
  private drawAlertBox(doc: PDFKit.PDFDocument, text: string, x: number, w: number, type: 'warning' | 'success') {
    const bgColor = type === 'warning' ? THEME.alertBg : THEME.alertBgGreen;
    const borderColor = type === 'warning' ? THEME.warning : THEME.success;
    ...
    doc.font(FONTS.body).fontSize(9.5).fillColor(THEME.textPrimary)
  ...
  private renderPredictionPage(
    ...
    // Intro text
    doc.font(FONTS.body).fontSize(10.5).fillColor(THEME.textSecondary)
      .text(localized.predictionBody, x, doc.y, { width: contentW, lineGap: 4, align: 'justify' });
    doc.moveDown(1.2);

    // Prediction status card
    const cardY = doc.y;
    const cardH = 90;
    doc.roundedRect(x, cardY, contentW, cardH, 6).fill(THEME.highlightBg);
    doc.roundedRect(x, cardY, contentW, cardH, 6).strokeColor(THEME.borderLight).lineWidth(1).stroke();

    // Status indicator
    const statusColor = input.prediction.status === 'COMPLETED' ? THEME.success : THEME.warning;
    doc.circle(x + 20, cardY + 20, 6).fill(statusColor);

    doc.font(FONTS.heading).fontSize(11).fillColor(THEME.primary)
    ...
    doc.text(`Prediction Available: ${input.prediction.hasPrediction ? 'Yes' : 'No'}`, x + 20, cardY + 54);
    if (input.prediction.createdAt) {
      doc.text(`Timestamp: ${new Date(input.prediction.createdAt).toISOString()}`, x + 20, cardY + 70);
    }
  ...
  private drawEnhancedBarChart(
    ...
    doc.roundedRect(x, y, width, height, 6).fill(THEME.pageBg);
    doc.roundedRect(x, y, width, height, 6).strokeColor(THEME.borderLight).lineWidth(1).stroke();

    // Chart title
    doc.font(FONTS.heading).fontSize(10).fillColor(THEME.primary)
    ...
    const barGap = 12;
    const barWidth = Math.max(16, (chartWidth - barGap * (data.length - 1)) / data.length);
    ...
    for (let i = 0; i <= 4; i++) {
      ...
      doc.moveTo(chartLeft, gridY).lineTo(chartRight, gridY)
        .strokeColor(THEME.borderLight).lineWidth(0.5).dash(3, { space: 3 }).stroke();
      doc.undash();

      doc.font(FONTS.body).fontSize(7).fillColor(THEME.textMuted)
        .text(this.formatNumber(gridValue), x + 8, gridY - 4, { width: 48, align: 'right' });
    }
    ...
    data.forEach((point, i) => {
      ...
      doc.rect(barX, barY, barWidth, barHeight).fill(THEME.chartBar);
      doc.rect(barX, barY, barWidth, 3).fill(THEME.accentLight); // highlight top

      // Value label on top
      if (point.value > 0) {
        doc.font(FONTS.heading).fontSize(7).fillColor(THEME.primary)
  ...
  // ═══════════════════════════════════════════════════════════════
  private drawEnhancedMetricsTable(doc: PDFKit.PDFDocument, rows: FinancialRow[], contentW: number) {
    const latestRows = [...rows]
      .sort((a, b) => new Date(b.period).getTime() - new Date(a.period).getTime())
      .slice(0, 18);

    if (latestRows.length === 0) {
      doc.font(FONTS.body).fontSize(10).fillColor(THEME.textMuted).text('No metrics available.');
      return;
    }

    const x = 56;
    let y = doc.y + 8;
    const colWidths = [Math.floor(contentW * 0.45), Math.floor(contentW * 0.27), Math.floor(contentW * 0.28)];
    const rowH = 24;

    // Header row
    doc.roundedRect(x, y, contentW, rowH, 3).fill(THEME.tableHeaderBg);
    doc.font(FONTS.heading).fontSize(9).fillColor(THEME.textWhite);
    doc.text('Metric', x + 10, y + 7, { width: colWidths[0] - 15 });
    doc.text('Value', x + colWidths[0] + 8, y + 7, { width: colWidths[1] - 12 });
    doc.text('Period', x + colWidths[0] + colWidths[1] + 8, y + 7, { width: colWidths[2] - 12 });
    ...
    latestRows.forEach((row, index) => {
      ...
      const bg = index % 2 === 0 ? THEME.pageBg : THEME.sectionBg;
      doc.rect(x, y, contentW, rowH).fill(bg);

      // Metric name
      doc.fillColor(THEME.textPrimary)
        .text(this.formatMetricName(row.metric), x + 10, y + 7, { width: colWidths[0] - 15 });

      // Value with color coding
      const numValue = Number(row.value) || 0;
      const valueColor = numValue < 0 ? THEME.danger : THEME.textPrimary;
      doc.fillColor(valueColor)
        .text(numValue.toLocaleString(), x + colWidths[0] + 8, y + 7, { width: colWidths[1] - 12 });

      // Period
      doc.fillColor(THEME.textSecondary)
        .text(new Date(row.period).toISOString().slice(0, 10), x + colWidths[0] + colWidths[1] + 8, y + 7, {
    ...
    // Bottom border
    doc.moveTo(x, y).lineTo(x + contentW, y).strokeColor(THEME.borderMedium).lineWidth(1).stroke();
    doc.y = y + 8;
  }
  ...
  private drawPageHeader(doc: PDFKit.PDFDocument, pageW: number) {
    ...
    doc.rect(0, 0, pageW, 3).fill(THEME.accent);
  ...
  private drawPageFooter(
    ...
    // Footer separator
    doc.moveTo(56, footerY).lineTo(pageW - 56, footerY)
      .strokeColor(THEME.borderLight).lineWidth(0.5).stroke();

    // Left: Brand
    doc.font(FONTS.heading).fontSize(7.5).fillColor(THEME.textMuted)
      .text('SmartBiz AI', 56, footerY + 8);

    // Center: generation source
    const poweredByLabel = this.getPoweredByLabel(aiSource);
    doc.font(FONTS.body).fontSize(7.5).fillColor(THEME.textLight)
    ...
    // Right: Page number
    doc.font(FONTS.heading).fontSize(8).fillColor(THEME.accent)
  ...
  // HELPER METHODS
  // ═══════════════════════════════════════════════════════════════
  private ensureSpace(doc: PDFKit.PDFDocument, needed: number, addPageFn: () => void) {
  ...
  private getLabels(language: string) {
    if (language === 'FR') {
      return {
        ...
        reportSubtitle: 'Analyse financière et stratégique générée par Intelligence Artificielle',
  ...
  private prettyType(type: string, lang: string): string {
    ...
    switch (type) {
      case 'FINANCIAL':
        return isFr ? 'Analyse Financière' : isAr ? 'التحليل المالي' : 'Financial Analysis';
      case 'BUSINESS_DESCRIPTION':
        return isFr ? 'Description de l\'Entreprise' : isAr ? 'وصف الشركة' : 'Business Description';
      case 'RISK_ANALYSIS':
        return isFr ? 'Analyse des Risques' : isAr ? 'تحليل المخاطر' : 'Risk Analysis';
      case 'ACTION_PLAN':
        return isFr ? 'Plan d\'Action Stratégique' : isAr ? 'خطة العمل الاستراتيجية' : 'Strategic Action Plan';
      default:
        return type;
    }
  }
  ...
  private prettySectionKey(key: string, lang: string): string {
    ...
    const isAr = lang === 'AR';
    switch (name) {
      case 'EXECUTIVE_SUMMARY': return isFr ? 'Résumé Exécutif' : isAr ? 'الملخص التنفيذي' : 'Executive Summary';
      case 'SWOT_ANALYSIS': return isFr ? 'Analyse SWOT' : isAr ? 'تحليل SWOT' : 'SWOT Analysis';
      case 'PERFORMANCE_ANALYSIS': return isFr ? 'Analyse de Performance' : isAr ? 'تحليل الأداء' : 'Performance Analysis';
      case 'FINANCIAL_OVERVIEW': return isFr ? 'Aperçu Financier' : isAr ? 'نظرة عامة مالية' : 'Financial Overview';
      case 'RECOMMENDATIONS': return isFr ? 'Recommandations' : isAr ? 'التوصيات' : 'Recommendations';
      case 'FORECASTS_TRENDS': return isFr ? 'Prévisions et Tendances' : isAr ? 'التوقعات والاتجاهات' : 'Forecasts & Trends';
  ...
  private getSectionIcon(type: string, lang: string): string {
    ...
    const isAr = lang === 'AR';
    switch (type) {
      case 'FINANCIAL': return isFr ? 'Examen détaillé des revenus, de la rentabilité et de la stabilité financière' : isAr ? 'مراجعة مفصلة للإيرادات والربحية والاستقرار المالي' : 'Detailed review of revenue, profitability, and financial stability';
      case 'BUSINESS_DESCRIPTION': return isFr ? 'Analyse de l\'état actuel de l\'entreprise et de sa position sur le marché' : isAr ? 'تحليل الوضع الحالي للشركة ومكانتها في السوق' : 'Analysis of current business state and market position';
      case 'RISK_ANALYSIS': return isFr ? 'Identification des principaux risques financiers et opérationnels' : isAr ? 'تحديد المخاطر المالية والتشغيلية الرئيسية' : 'Identification of key financial and operational risks';
      case 'ACTION_PLAN': return isFr ? 'Recommandations concrètes basées sur les données et prochaines étapes' : isAr ? 'توصيات ملموسة مبنية على البيانات والخطوات التالية' : 'Concrete, data-driven recommendations and next steps';
      default: return '';
    }
  }
