...
export class FinancialService {
  ...
  async processBimodalImport(fileBuffer: Buffer, companyId: string) {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      if (!rawKPIs)
        throw new BadRequestException('Strategic KPIs sheet is empty');
      ...
      if (kpiErrors.length > 0)
        throw new BadRequestException(
          'Strategic KPIs sheet has invalid data format',
        );

      // 3. Parse Sheet 1: Valuation Annual
      const valuationData = XLSX.utils.sheet_to_json(
        workbook.Sheets['Valuation_Annual'],
      )[0];
      if (!valuationData)
        throw new BadRequestException('Valuation Annual sheet is empty');
      ...
      const rawCashflow = XLSX.utils.sheet_to_json(
        workbook.Sheets['CashFlow_Monthly_TTM'],
      );
      ...
      for (const row of rawCashflow as any[]) {
        ...
        for (const key of Object.keys(row)) {
          ...
          if (errors.length > 0) {
            throw new BadRequestException(
              `Validation failed on CashFlow sheet for metric: ${metric} at period ${key}`,
            );
          }
  ...
  generateTemplate(): Buffer {
    ...
    const cashFlowData = metricsList.map((metric) => ({
      ...monthHeaders,
      Metric: metric,
    }));
  ...
  async getDashboardMetricsByBatchId(batchId: string, companyId: string) {
    ...
    if (batch.companyId !== companyId) {
      throw new ForbiddenException(
        'You do not have permission to access this import batch',
      );
    }
